...
### 6. Προειδοποίηση Recovery Model
...
Αντικαταστήστε τα placeholders στο Βήμα 1 του script με τα δικά σας στοιχεία, αφαιρέστε τα σχόλια και εκτελέστε:
...
CREATE CREDENTIAL [https://YOUR_STORAGE_ACCOUNT.blob.core.windows.net/YOUR_CONTAINER]
    ...
    SECRET = N'sp=...'   -- Εισάγετε το SAS token σας (χωρίς το αρχικό '?')
GO
````````
...
- **Επιλογή Β:** Αφαίρεση σχολίου από τη γραμμή `EXEC(@SQL)` μέσα στον cursor loop για αυτόματη εκτέλεση
...
---
*Αυτό το script δημιουργεί μόνο εντολές — αφαιρέστε το σχόλιο από τη γραμμή `EXEC(@SQL)` μέσα στον cursor loop για αυτόματη εκτέλεση.*
